# Sub2API v0.1.170-watch.1 发布与切流验证报告 v6

状态：verified / accepted，生产已切换至绿色，蓝色保留
...
- 用户另行明确确认后，本轮包含 Nginx 从蓝色 `8093` 到绿色 `8094` 的切流。
- 本轮不包含停蓝、启用故障转移、退款、余额调整或数据库恢复。
...
| Nginx 切流 | 配置测试通过后 reload；当前唯一目标 `127.0.0.1:8094` | 通过 |
| 切流观察 | 55 个新增请求全部 200；新增 5xx=0、绿色 error=0 | 通过 |
| 计费观察 | 98 条新增 usage；failover attempt=0、失败 attempt 结算=0 | 通过 |
| 回滚能力 | 切流前 Nginx 配置独立备份；蓝色 `8093` 保持 healthy | 通过 |
...
- `/usr/bin/nginx -t`、Nginx reload、公网 health/login、切流后日志状态分布和数据库安全线核对：通过。
