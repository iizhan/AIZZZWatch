...
- 增量边界：新增 GitHub Actions 原生 Windows 构建与临时 Artifact；首次验证仅允许当前功能分支的受限 `push` 自动触发，后续默认分支仅手动触发。
...
- CI 权限影响：工作流声明 `contents: read`；不注入 secrets、不上传运行数据，仅上传 `release/*.exe`。
...
| ITEM-PKG-005 | TASK-PKG-005..008 | TASK-PKG-002 | Windows 原生 CI 生成可下载 EXE Artifact | `.github/workflows`、README、specs | YAML 审阅、Actions Run、Artifact 下载记录 |
...
| Windows CI 构建 | GitHub `windows-latest` | Actions run、EXE Artifact | 工作流失败或 Artifact 缺失 |
...
- 风险 3：Actions Artifact 构建不能替代 Windows 安装、托盘、窗口形态和卸载的可见验证。
...
- 任务拆解版本：v3
- 影响范围版本：v3
- 验收与自测计划：verify、DMG 构建/挂载/启动、Windows CI 原生构建与 Artifact、Windows 实机安装/托盘/卸载待验收、差异审查。
- 用户确认：2026-07-24 确认执行 Windows CI 打包增量 v3
