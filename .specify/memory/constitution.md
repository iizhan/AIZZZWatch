...
当用户质疑 UI 文案、按钮动作、状态标签或指标名称的“业务含义”时，必须先确认语义再实现。涉及账号、分组、来源站点、成本、倍率、充值比例、利润、隐藏/删除/停用等领域词时，文案视为业务行为的一部分；即使代码改动很小，也不得按 fast 直接修改。
...
实现必须遵循 `Electron + React + TypeScript` 与 `.` 的实际约定。
...
- 有重大歧义时暂停，不直接动手
- 有业务语义歧义时发布 `语义确认 vN` / `思考确认 vN`，说明术语定义、影响状态、非目标和验收文案；用户确认前不得编辑
