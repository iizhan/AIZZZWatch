...
# 2026-08-03 Sub2API v0.1.170-watch.1 蓝绿切流

- 授权：用户明确回复“确认切流”；切流范围仅为 Nginx 从蓝色 `8093` 指向绿色 `8094`，蓝色继续保留。
- 完成：切流前蓝绿 healthy、备份存在、故障转移关闭、异常结算增量为 0；Nginx 单文件变更通过配置测试并 reload，公网 health/login 正常。
- 观察：约一分钟内新增 55 个 Nginx 请求全部为 200，新增 5xx=0、绿色错误日志=0；数据库新增 98 条 usage、0 个 failover attempt、0 个失败-attempt 结算。
- 回滚：切流前配置保存在 `/www/wwwroot/sub2api-deploy/backups/20260803T101649Z-nginx-8093-to-8094`；蓝色容器仍为 healthy，未停止。
- 剩余风险：首分钟样本有限，需继续观察流式和高并发路径；停止蓝色、启用故障转移、退款和数据库恢复仍需独立授权。
