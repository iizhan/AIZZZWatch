...
func registerWatchRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	watch := admin.Group("/watch")
	{
		...
		watch.GET("/rate-anomalies", h.Admin.Watch.ListRateAnomalies)
		watch.GET("/rate-anomalies/:id/compensation-preview", h.Admin.Watch.PreviewRateCompensation)
		watch.POST("/rate-anomalies/:id/compensate", h.Admin.Watch.ApplyRateCompensation)
		watch.POST("/rate-compensations/external", h.Admin.Watch.RecordExternalRateCompensation)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// 网关账号故障转移配置
		adminSettings.GET("/gateway-failover", h.Admin.Setting.GetGatewayFailoverSettings)
		adminSettings.PUT("/gateway-failover", h.Admin.Setting.UpdateGatewayFailoverSettings)
		// 面板 API 限流配置
...
func registerUsageRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	usage := admin.Group("/usage")
	{
		...
		usage.GET("/failover-refunds/dry-run", h.Admin.Usage.DryRunGatewayFailoverRefunds)
		usage.POST("/failover-refunds/candidates", h.Admin.Usage.StageGatewayFailoverRefundCandidates)
