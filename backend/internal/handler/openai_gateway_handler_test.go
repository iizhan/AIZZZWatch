...
func TestOpenAIResponses_GatewayFailoverDefaultsToDisabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	groupID := int64(4203)
	accounts := []service.Account{
		{
...
func TestOpenAIResponses_APIKeyPassthroughPoolAuthFailureRetriesThenSwitchesToHealthyAccount(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			billingCacheSvc := service.NewBillingCacheService(nil, nil, nil, nil, nil, nil, cfg, nil)
			t.Cleanup(billingCacheSvc.Stop)
			settingSvc := service.NewSettingService(gatewayFailoverAuthSettingRepo{}, cfg)
			gatewaySvc := service.NewOpenAIGatewayService(
				accountRepo,
				nil,
				nil,
...
type gatewayFailoverAuthSettingRepo struct {
	service.SettingRepository
}

func (gatewayFailoverAuthSettingRepo) GetValue(_ context.Context, key string) (string, error) {
	if key == service.SettingKeyGatewayFailoverSettings {
		return `{"enabled":true,"status_codes":"401,403","max_account_switches":1}`, nil
	}
	return "", service.ErrSettingNotFound
}
