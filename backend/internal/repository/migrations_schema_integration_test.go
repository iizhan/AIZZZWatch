...
func TestMigrationsRunner_IsIdempotent_AndSchemaIsUpToDate(t *testing.T) {
	...
	// Watch source data remains additive and isolated from official account/group tables.
	...
	requireIndex(t, tx, "watch_sources", "idx_watch_sources_due")
	requireIndex(t, tx, "watch_source_prices", "idx_watch_source_prices_rank")
	requireColumn(t, tx, "watch_price_changes", "next_value", "numeric", 0, false)
	requireIndex(t, tx, "watch_price_changes", "idx_watch_price_changes_source_observed")
	requireIndex(t, tx, "watch_price_changes", "idx_watch_price_changes_observed")
	requireColumn(t, tx, "watch_price_changes", "source_name_snapshot", "text", 0, false)
	requireColumn(t, tx, "watch_price_changes", "group_name_snapshot", "text", 0, false)
	requireColumn(t, tx, "watch_source_keys", "external_id", "character varying", 128, false)
	...
	requireIndex(t, tx, "watch_source_price_history", "idx_watch_source_price_history_lookup")
