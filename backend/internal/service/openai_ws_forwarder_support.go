...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	// Responses state is valid on both HTTP and WebSocket transports. Keeping
	// the response-to-account binding prevents a stateful HTTP turn from being
	// replayed against a different upstream account.
